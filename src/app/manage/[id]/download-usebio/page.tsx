...
export default function DownloadUsebioPage() {
  ...
  async function handleDownload(e: React.FormEvent) {
    ...
    try {
      ...
      const usebioRes = await fetch(`/api/games/${gameId}/usebio`);
      if (!usebioRes.ok) {
      ...
      // Navigate back to menu after download
    ...
    <div className="min-h-dvh flex flex-col bg-white">
      ...
      <form onSubmit={handleDownload} className="flex-1 flex flex-col px-6 pt-6 pb-8 max-w-sm w-full mx-auto">
        ...
        <div className="flex flex-col gap-3 pt-6">
          ...
          <button
            type="button"
            onClick={() => router.replace(`/manage/${gameId}/menu`)}
            disabled={saving}
