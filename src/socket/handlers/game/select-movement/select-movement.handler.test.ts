...
vi.mock(
  "@/db/games/individual/actions/create-player-movement-map-entry",
  () => ({
...
describe("registerSelectedMovementHandler (unit)", () => {
  ...
  it("handles invalid type gracefully (TEAM fallback)", async () => {
    ...
    await handler({ gameId: "g1", type: "TEAM", id: 10 }, cb);
  ...
  it("returns failure on error", async () => {
    ...
    (getIndividualMovement as any).mockRejectedValue(new Error("fail"));
