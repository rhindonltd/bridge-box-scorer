...
import { assignments as individualAssignments } from "@/db/games/individual/tables/assignments";
import { participants as pairParticipants } from "@/db/games/pairs/tables/participants";
import { participants as individualParticipantsTable } from "@/db/games/individual/tables/participants";
...
export async function getPlayerSchedule(gameId: string, gameType: string, seat: string) {
  if (gameType === "INDIVIDUAL") {
    return getIndividualSchedule(gameId, seat);
  } else {
    return getPairsSchedule(gameId, seat);
  }
}

async function getIndividualSchedule(gameId: string, seat: string) {
  const db = await getIndividualDb(gameId);

  // Look up assignment ID for this seat
  const assignment = await db
    .select()
    .from(individualAssignments)
    .where(eq(individualAssignments.initialSeat, seat as any))
    .get();
  ...
  const direction = seat.slice(-1); // N, S, E, or W
  const side: "NS" | "EW" = direction === "N" || direction === "S" ? "NS" : "EW";

  // Query all boards where this player appears in any position
  const allBoards = await db
    .select()
    .from(individualBoards)
    .where(
      or(
        eq(individualBoards.n, assignmentId),
        eq(individualBoards.s, assignmentId),
        eq(individualBoards.e, assignmentId),
        eq(individualBoards.w, assignmentId),
      ),
    );
  ...
  // Build player lookup for individual
  const allIndAssignmentRows = await db.select().from(individualAssignments);
  const allIndParticipantRows = await db.select().from(individualParticipantsTable);
  const allIndPlayerRows = await db.select().from(players);

  const indPlayerById = new Map(allIndPlayerRows.map((p) => [p.id, p]));
  const indAssignmentSeatMap = new Map(allIndAssignmentRows.map((a) => [a.id, a.initialSeat]));

  // Map: initialSeat -> Player
  const indSeatToPlayer = new Map<string, typeof allIndPlayerRows[0]>();
  for (const p of allIndParticipantRows) {
    const player = indPlayerById.get(p.player);
    if (player && p.initialSeat) {
      indSeatToPlayer.set(p.initialSeat, player);
    }
  }

  // Map: assignment ID -> Player
  const indAssignmentToPlayer = new Map<string, typeof allIndPlayerRows[0]>();
  for (const [id, initialSeat] of indAssignmentSeatMap) {
    const player = initialSeat ? indSeatToPlayer.get(initialSeat) : undefined;
    if (player) {
      indAssignmentToPlayer.set(id, player);
    }
  }

  // Per round, get n/s/e/w assignment IDs from the first board
  const roundPlayers = new Map<number, { n: string; s: string; e: string; w: string }>();
  for (const b of allBoards) {
    if (!roundPlayers.has(b.roundNumber)) {
      roundPlayers.set(b.roundNumber, { n: b.n, s: b.s, e: b.e, w: b.w });
    }
  }
  ...
  const rounds = Array.from(roundMap.entries())
    ...
    .map(([roundNumber, data]) => {
      const seats = roundPlayers.get(roundNumber);
      return {
        roundNumber,
        ...
        players: {
          N: seats ? indAssignmentToPlayer.get(seats.n) ?? null : null,
          S: seats ? indAssignmentToPlayer.get(seats.s) ?? null : null,
          E: seats ? indAssignmentToPlayer.get(seats.e) ?? null : null,
          W: seats ? indAssignmentToPlayer.get(seats.w) ?? null : null,
        },
      };
    });
  ...
  return { assignmentId, side, rounds: completeIndRounds };
}
