...
import { LobbyEvent, LobbyClientToServerEvents, LobbyServerToClientEvents } from "@/types/lobbySocket"

export function useLobbySocket() {
    const socketRef = useRef<Socket<LobbyServerToClientEvents, LobbyClientToServerEvents> | null>(null)
    ...
    useEffect(() => {
        ...
        socket.on("event:created", (newEvent: LobbyEvent) => {
            ...
            mutate("/api/events", (events: LobbyEvent[] = []) => [...events, newEvent], false)
        })
