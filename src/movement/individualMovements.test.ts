...
import { generateIndividualMovements } from "./individualMovements";

describe("generateIndividualMovements", () => {
  const movements = generateIndividualMovements();
  ...
  it("each round has INDIVIDUAL participant fields (nId, sId, eId, wId)", () => {
    const m = movements[0];
    for (const table of m.tableData) {
      for (const round of table.rounds) {
        expect(round.participants).toHaveProperty("nId");
        expect(round.participants).toHaveProperty("sId");
        expect(round.participants).toHaveProperty("eId");
        expect(round.participants).toHaveProperty("wId");
      }
    }
  });
