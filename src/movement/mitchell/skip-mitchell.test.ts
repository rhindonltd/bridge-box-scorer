...
describe("generateSkipMitchell", () => {
  ...
  it("does not visit the skipped table position", () => {
    ...
    const opponents = table1?.rounds.map((round) => round.participants.ewId);
  ...
  it("rejects an odd number of tables", () => {
    ...
    ).toThrow("Skip Mitchell requires an even number of tables");
  });
  ...
  it("rejects a number of rounds equal to the number of tables", () => {
    ...
    ).toThrow("Skip Mitchell must have fewer rounds than tables");
  });
