...
describe("generateShareAndRelayMitchell", () => {
  ...
  it("has the first and last tables share the same boards", () => {
    ...
    const table6 = result.tables.find((table) => table.table === 6);

    expect(table1?.rounds.map((round) => round.boards)).toEqual(
  ...
  it("uses the relay between the two halves of the movement", () => {
    ...
    const table2 = result.tables.find((table) => table.table === 2);

    const table3 = result.tables.find((table) => table.table === 3);

    const table4 = result.tables.find((table) => table.table === 4);

    const table5 = result.tables.find((table) => table.table === 5);
    ...
    expect(table1?.rounds[0].boards).toEqual([1, 2, 3, 4]);

    expect(table2?.rounds[0].boards).toEqual([5, 6, 7, 8]);

    expect(table3?.rounds[0].boards).toEqual([9, 10, 11, 12]);

    expect(table4?.rounds[0].boards).toEqual([17, 18, 19, 20]);

    expect(table5?.rounds[0].boards).toEqual([21, 22, 23, 24]);

    expect(table6?.rounds[0].boards).toEqual([1, 2, 3, 4]);
  });
  ...
  it("gives every NS pair every board set exactly once", () => {
    ...
    for (const table of result.tables) {
      const boardSets = table.rounds.map((round) => round.boards.join(","));
  ...
  it("shares boards between tables 1 and 10 with 10 tables", () => {
    ...
    const table10 = result.tables.find((table) => table.table === 10);
  ...
  it("rejects an odd number of tables", () => {
    ...
    ).toThrow("Share and Relay Mitchell requires an even number of tables");
  });
  ...
  it("supports a 1-winner arrow switch", () => {
    ...
    // exchange directions.
    const lastRound = table1?.rounds[5].participants;
