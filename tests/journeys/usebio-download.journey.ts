...
test("USEBIO XML download with club data", async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await test.step(
      "Set up club information via API",
      async () => {
    ...
    await test.step(
      "Director navigates to USEBIO download page",
      async () => {
    ...
    await test.step(
      "Club Name and EBU Club Number fields are pre-populated",
      async () => {
    ...
    await test.step(
      "Download USEBIO button triggers XML download",
      async () => {
