...
test.describe("Download USEBIO", () => {
  test("download-usebio page renders", async ({ directorContext }) => {
    ...
    await page.route("**/api/system/club", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          club: { name: "Test Club", clubNumber: "99999" },
        }),
      }),
    );
    ...
    await expect(
      page
        .getByRole("heading", { name: "Download USEBIO" })
        .or(page.locator("div").filter({ hasText: /^Download USEBIO$/ })),
  ...
  test("Download USEBIO button triggers API call", async ({
    directorContext,
  }) => {
    const { page, gameId } = directorContext;
    ...
    await page.route("**/api/system/club", (route) => {
      if (route.request().method() === "GET") {
        return route.fulfill({
          ...
          contentType: "application/json",
          body: JSON.stringify({
            club: { name: "Anytown BC", clubNumber: "12345" },
          }),
        });
      }
    ...
    await expect(page.getByLabel("Club Name")).toHaveValue("Anytown BC", {
      timeout: 10000,
    });
  ...
  test("error display when USEBIO generation fails", async ({
    directorContext,
  }) => {
    const { page, gameId } = directorContext;
    ...
    await page.route(`**/api/games/${gameId}/usebio`, (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({
          success: false,
          error: "Club info not configured",
        }),
      }),
    );
    ...
    await expect(page.getByText("Club info not configured")).toBeVisible({
      timeout: 10000,
    });
  });
