...
- **Cross-bundle server singletons must live on `globalThis`.** The app ships as two separate bundles that each get their own copy of every module: the custom server (`server.ts` → esbuild `dist/server.js`, which owns Socket.IO and the timer machinery) and the Next.js route handlers (`next build` → `.next/server`, which run API routes). A module-level `let`/`const` therefore exists once _per bundle_, so state written by a route handler is invisible to the socket server and vice versa (and dev-mode HMR also wipes module state). Any in-process server state shared between HTTP routes and the socket server MUST be stored on `globalThis` behind a namespaced key (`__bridgeBox*`). Existing examples: the Socket.IO server (`src/socket/websocket.ts`, `getIO()`), the in-memory timer engine store (`src/timer/game-store.ts`), and the phase scheduler (`src/timer/scheduler.ts`). This works because the appliance runs a single self-hosted process; it would not share state across a multi-instance/serverless deployment. Symptom when you get this wrong: an HTTP mutation succeeds and the DB updates, but the live socket state (or a cached engine) is stale until a full re-read.
